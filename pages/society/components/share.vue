			...
			uni.getImageInfo({
				...
				success: res => {
					...
					this.$nextTick(() => {
						...
						let share_text="";
						let row_text_num=23;//一行字数
						let text_len=Math.ceil(this.share_text.length/row_text_num);						
						for (let i=0;i<text_len;i++) {
							share_text=this.share_text.slice(i*row_text_num,i*row_text_num+row_text_num);
							if(i>=3&&share_text.length>18){
								share_text=share_text.slice(0,18);
								share_text+="..."
							}
							ctx.fillText(share_text, 30, this.img_height - (195-i*18)); //参数 文字 left top		
							if(i>=3){
								break;
							}
						}
						...
						ctx.fillText('我的邀请码: ' + this.user_info.user_code, 30, this.img_height - 95); //参数 文字 left top
						ctx.font = `${12}px PingFang`; // 文字类型
						ctx.fillText('嗨，我是' + this.user_info.nickname, 30, this.img_height - 75); //参数 文字 left top
						ctx.fillText('① 微信/浏览器扫码，', 30, this.img_height - 55); //参数 文字 left top
						ctx.fill();
