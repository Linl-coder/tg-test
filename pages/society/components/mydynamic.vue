	...
	<view class="hot_dynamic">
		<view class="hot_dynamic_box" @tap="topage('society/deatils?id='+item.id)" v-for="(item,index) in list"
			...
			<view class="user_info flex-start" >
				<image @click.stop="topage('gather/collect/author?author_id='+item.user.id)" :src="user_info.avatar?user_info.avatar:'/static/else/avatar.png'" class="avatar" mode=""></image>
			...
			<!-- 投票 -->
			<view class="vote-box"  v-if="item.type==3&&item.vote">
				<vote :list="item.vote" v-if="item.vote.length>0">
