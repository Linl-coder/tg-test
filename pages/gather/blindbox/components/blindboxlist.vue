<template>
	<view class="collect-list" :id="'collect-list' + current">
		<view v-show="show" class="list-one" @tap="topage('gather/blindbox/details?id=' + item.id)"  v-for="(item, index) in list" :key="index">
			<view class="icon-box">
	...
	methods:{
		...
		async set_img_info(){
			...
			this.list.forEach((item, index) => {
				item.cover.imgheight = parseFloat(uni.utils.set_imgh(item.cover.width, item.cover.height, img_info.width * 0.94)).toFixed(2) + 'px';	
