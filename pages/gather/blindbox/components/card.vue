			...
			<view class="mask">
				<view class="top-box" id="top-box">
					<view class="card_item flex-between" v-if="popup_show">
						<view class="icon-box"><u--image :showLoading="true" :src="info.box.cover.show_src"
					...
					<view class="desc flex-between">
						...
						<text class="t1" @tap="topage('gather/blindbox/boxHistory?box_id='+info.box_id)">开盒记录</text>
					</view>
				</view>
						...
						<view class="card" @tap="to_details(item)"
							...
							<view class="card_box flex-colum">
								...
								<view class="tips  flex-center" :class="{'tips_active':item.status==2}">
									<text class="t">{{set_status(item.status)}}</text>
								</view>
							</view>
						</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			async get_data() {
				let res = await uni.ajax.collect_getMyListInfobox({
					box_id: this.id,
					page: this.page,
					limit: this.limit,
				})
