let c_num = 0;
let typeName = getUrl('type');

function Countdown() {
	// 设置默认参数
	this.settings = {
		id: "canvas", // ID，canvas一定要有ID属性
		size: 180, // 绘制圆形的最大尺寸，宽=高
		borderWidth: 4, // 边框宽度
...
Countdown.prototype.drawAnimate = function() {
	...
	this.ctx.arc(this.settings.size / 2, this.settings.size / 2, this.settings.size / 2 - 3, startAng * deg,
		endAng * deg, false);
...
Countdown.prototype.drawCircle = function(startAng, endAng, border, fillColor) {
	let deg = Math.PI / 180;
	this.ctx.beginPath();
	this.ctx.arc(this.settings.size / 2, this.settings.size / 2, this.settings.size / 2 - border, startAng * deg,
		endAng * deg, false);
...
Countdown.prototype.countdown = function() {
	...
	timer = setInterval(() => {
		...
		if (currentTime - oldTime >= allMs) {
			...
			if (c_num >= this.settings.count) {
				//  重绘
				this.drawBackground();
				this.drawProcess();
				this.drawAnimate();
				this.drawInner();
				this.strokeText(this.settings.count);
			} else {
				this.countdown();
			}
		}
